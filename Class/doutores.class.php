	...
	class Doutores {
		...
		function add($redireciona='') {
			if(isset($_POST['acao']) && $_POST['acao'] == 'addDoutor') {
				...
				$breve2 = filter_input(INPUT_POST, 'breve2');
				...
				$titulo6 = filter_input(INPUT_POST, 'titulo6');
				$titulo7 = filter_input(INPUT_POST, 'titulo7');
				$titulo8 = filter_input(INPUT_POST, 'titulo8');
                ...
                if($_FILES["foto"]["size"] > $maximo) {
                 ...
                 // Verificação
                if($_FILES["foto_1"]["size"] > $maximo) {
                    echo "Erro! O arquivo enviado por você ultrapassa o ";
                    $valorKb = $maximo / 1000;
                    $tamanhoFoto = $_FILES["foto_1"]["size"] /1000;
                    echo "<script>
                    alert('limite máximo de " . $valorKb . " KB! Envie outro arquivo. Sua imagem tem ".$tamanhoFoto." KB');
                    history.back();
                    </script>";
					...
					try{
						...
						$sql = "INSERT INTO tbl_doutores (foto, nome, especialidade, curriculo, sexo, meta_title, meta_keywords, meta_description, graduacao, linguagem, dias_trabalho, telefone, email, url_amigavel, breve, instagram, facebook, linkedin, twitter, pertence_ao, id_categoria, cargo, ordem, id_hierarquia, foto_1, titulo2, breve2, titulo3, titulo4, titulo5, titulo6, titulo7, titulo8) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? ,? , ?, ?)";   
						$stm = $this->pdo->prepare($sql);   
						...
						$stm->bindValue(25, upload('foto_1', $pastaArquivos, 'N')); 
						$stm->bindValue(26, $titulo2);
						$stm->bindValue(27, $breve2);
						$stm->bindValue(28, $titulo3);
						$stm->bindValue(29, $titulo4);
						$stm->bindValue(30, $titulo5);
						$stm->bindValue(31, $titulo6);
						$stm->bindValue(32, $titulo7);
						$stm->bindValue(33, $titulo8);
						$stm->execute(); 
		...
		function editar() {
			if(isset($_POST['acao']) && $_POST['acao'] == 'editaDoutor') {
				...
				try { 
					...
					$sql = "UPDATE tbl_doutores SET foto=?, nome=?, especialidade=?, curriculo=?, sexo=?, meta_title=?, meta_keywords=?, meta_description=?, graduacao=?, linguagem=?, dias_trabalho=?, telefone=?, email=?, url_amigavel=?, breve=?, instagram=?, facebook=?, linkedin=?, twitter=?, pertence_ao=?, id_categoria=?, cargo=?, ordem=?, id_hierarquia=?, foto_1=?, titulo2=?, breve2=?, titulo3=?, titulo4=?, titulo5=?, titulo6=?, titulo7=?, titulo8=? WHERE id=?";   
					$stm = $this->pdo->prepare($sql);   
					$stm->bindValue(1, upload('foto', $pastaArquivos, 'N'));   
					$stm->bindValue(2, $nome);   
					$stm->bindValue(3, $especialidade);   
