			...
			if(isset($_POST['acao']) && $_POST['acao'] == 'addBanner') {
				...
				$tem_video 	= filter_input(INPUT_POST, 'tem_video');
					...
					try{
						...
						$sql = "INSERT INTO tbl_sliders (foto, titulo1, titulo2, breve, tem_botao, nome_botao, link_botao, lado_texto, imagem_mobile, embed, tem_video) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";   
						$stm = $this->pdo->prepare($sql);   
		...
		function editar($redireciona='banners.php') {
			if(isset($_POST['acao']) && $_POST['acao'] == 'editaBanner') {
				...
				try { 
					...
					$sql = "UPDATE tbl_sliders SET foto=?, titulo1=?, titulo2=?, breve=?, tem_botao=?, nome_botao=?, link_botao=?, lado_texto=?, imagem_mobile=?, embed=?, tem_video=? WHERE id=?";   
					$stm = $this->pdo->prepare($sql);   
					$stm->bindValue(1, upload('foto', $pastaArquivos, 'N'));   
					$stm->bindValue(2, $titulo1);   
					$stm->bindValue(3, $titulo2);   
